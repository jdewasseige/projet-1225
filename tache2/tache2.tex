...
Comme $n_{H_2}=3*n_{N_2}$ (éléments en quantité stœchiométrique), on a donc $K==\frac{{n_{NH_3,out}}^2}{27{n_{N_2,out}}^4}*{n_{total,out}}^2$. En injectant les quantités de composés du tableaux, fonctions de $\xi$, on obtient:\\
$$K=\frac{{2\xi}^2}{27{{n_{N2}-\xi}^4} {n_{total}-2\xi}^2$$
Nous pouvons par exemple résoudre cela avec $MATLAB$. Connaissant $\xi$, on trouve \dot{n}_{purge} grâce à l'équation(5). Nous obtenons directement $[Ar]_{purge}$ en injectant le résultat dans la première équation. $[Ar]_{purge}$ est égal à $[Ar]_{recyclage}$ car la purge n'altère par la concentration.\\
