...
Après quelques recherches sur internet \cite{catalyseur}, nous avons découvert que les catalyseurs utilisés
pour le production d'ammoniac en milieu industriel contenaient principalement du fer (type \ce{Fe3O4}). 
...
cette réaction à basse température comme le suggérait nos premières observations.
...
le système à l'équilibre. Les flux y sont donc constants. Notre but est ici de calculer le rendement du circuit en terme de production d'ammoniac. Nous cherchons donc la quantité d'\ce{NH3} réellement produite.
...
ce qui nous donne tout le nécessaire pour résoudre le problème! On peut en effet alors
...
de manière plus conséquente avec la température. Les graphes \ref{fig:efficienceS} et \ref{fig:efficienceC} nous montrent cela.
