...
\[
K_p=\frac{{[NH_3]_{out}}^2}{[N_2]_{out}{[H_2]_{out}}^3} \cdot p = 
\frac{{n_{NH_3,out}}^2}{n_{N_2,out}\cdot {n_{H_2,out}}^3}\cdot {n_{total,out}}^2\cdot p
\]

Comme $n_{H_2}=3\cdot n_{N_2}$ (éléments en quantité stœchiométrique), on a donc 
$K=\frac{{n_{NH_3,out}}^2}{27{n_{N_2,out}}^4}\cdot {n_{total,out}}^2\cdot p$. En injectant les 
quantités des composés du tableaux, fonctions de $\xi$, on obtient:

\[ K=\frac{{2\xi}^2}{27{{n_{\ce{N2}}-\xi}^4}}\cdot ({n_{total}-2\xi})^2 \cdot p \]
...
\[ K=\frac{{n_{NH_3,out}}^2}{27{n_{N_2,out}}^4}\cdot {n_{total,out}}^2\cdot p \]

Or nous savons que $\dot{n_{total,out}}=4\dot{n_{N_2,out}}+\dot{n_{Ar,out}}+\dot{n_{\ce{NH3},out}}$ 
où $\dot{n}_{Ar,out}$ est successivement:
\[\dot{n}_{Ar,out}=\dot{n}_{Ar,in}+\dot{n}_{Ar,recyclage}\]

\[ \dot{n}_{Ar,out}=\dot{n}_{Ar,in}+[Ar]_{recyclage}\dot{n}_{total,recyclage} \]

\[ \dot{n}_{Ar,out}\underset{(3)}=\dot{n}_{Ar,in}+[Ar]_{recyclage}(\dot{n}_{total,out}-\dot{n}_{NH_3,out}) \].
